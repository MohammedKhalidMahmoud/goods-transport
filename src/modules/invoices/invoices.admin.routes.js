...
router.post('/invoices', ...tenant, authorizePermissions(PERMISSIONS.INVOICES_CREATE), validate({ body: Joi.object({ orderId: Joi.string().uuid().allow(null), providerId: Joi.string().uuid().allow(null), subtotal: Joi.number(), taxAmount: Joi.number(), totalAmount: Joi.number(), currency: Joi.string(), dueDate: Joi.date().iso().allow(null), notes: Joi.string().allow('', null), items: Joi.array().items(Joi.object({ description: Joi.string().required(), quantity: Joi.number(), unitPrice: Joi.number().required(), amount: Joi.number().required() })) }) }), invoicesController.createInvoice);
